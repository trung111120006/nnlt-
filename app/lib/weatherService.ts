...
export async function fetchWeatherAndAirQuality(location: string, includeForecast: boolean = true): Promise<WeatherResponse> {
  ...
  // 1. Fetch weather data from OpenWeatherMap by city name
...
export async function fetchWeatherAndAirQualityByCoords(
  lat: number,
  lon: number,
  ...
  const weatherUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${WEATHER_API_KEY}&units=metric&lang=vi`;
  const weatherResponse = await fetch(weatherUrl);
  ...
  if (includeForecast) {
    try {
      ...
      console.warn('Weather forecast fetch failed (coords):', error);
    }
  }
  ...
  if (AIR_QUALITY_API_KEY) {
    try {
      ...
      console.warn('Failed to fetch air quality from IQAir (coords):', error);
    }
  }
